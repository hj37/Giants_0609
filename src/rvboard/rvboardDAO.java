	...
	//커넥션풀(DataSource)을 얻은 후 ConnecionDB접속
		...
		public void insertrvboard(rvboardDTO dto) {
			...
			try {
				...
				//SQL문 만들기
				sql = "select max(ref) from rvboard where fk_num =? "; //가장 큰 글번호 검색 
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, dto.getFk_num());
				rs = pstmt.executeQuery(); //검색 후 값 얻기 
				
				if(rs.next()) {//가장 큰 글번호가 검색되면 
					//가장 큰 글번호에 + 1한 글번호를 ? 새글의 글번호로 사용하기 위해 저장 
					num = rs.getInt("max(ref)") + 1; //가장 최신 번호 검색하기 위해 
				...
				//insert SQL문만들기 
				sql = "insert into rvboard(ref,name,content,"
						+ "reg,id,fk_num)"
						+ "values(?,?,?,now(),?,?)";
		...
		public void deletervboard(int ref) {
			String sql = "";
			...
			try {
				...
				sql = "delete from rvboard where ref=?"; 
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, ref);
				pstmt.executeUpdate();	
				
			}catch (Exception e) {
				System.out.println("deletervboard메서드 내부에서 예외발생하였습니다:" +e.getMessage());
			}finally {
				try {
					if(rs != null) {rs.close();}
					if(pstmt != null) {pstmt.close();}
		...
		public List<rvboardDTO> getrvboard(int fk_num) {
			...
			try {
				...
				String sql = "select * from rvboard where fk_num = ? order by ref ";	
				//?기호 해당되는 값을 제외한 나머지 SELECT문장을 저장한? PreparedStatement실행 객체 얻기
				pstmt = con.prepareStatement(sql);
				//?기호에 해당되는 글번호를 설정
				pstmt.setInt(1,fk_num);
				...
				while(rs.next()) {
					...
					dto.setFk_num(rs.getInt("fk_num"));
		...
		//글 하나의 정보를 검색하여 글정보를 제공해주는 메소드 
		public rvboardDTO getrvBoard(int fk_num) {
			...
			try {
				...
				//매개변수로 전달 받은 글번호에 해당 되는 글 하나의 정보를 검색하는 SELECT구문 만들기 
				String sql = "select max(ref) from rvboard where fk_num = ?";	
				//?기호 해당되는 값을 제외한 나머지 SELECT문장을 저장한? PreparedStatement실행 객체 얻기
				pstmt = con.prepareStatement(sql);
				//?기호에 해당되는 글번호를 설정
				pstmt.setInt(1, fk_num);
				...
				if(rs.next()) {
					...
					num = rs.getInt("max(ref)");
					System.out.println(num);
				}else {
					System.out.println("못 찾았습니다.");
				}
				
				String sql2 = "select * from rvboard where ref = ?";
